...
You are given a table $m \times n$. Rows are numbered top to bottom from 1 to $n$, and columns are numbered left to right from 1 to $m$. Cell $(i, j)$ contains the value $(i-1) \cdot m + j$.

A turtle starts at $(1, 1)$ and wants to reach $(n, m)$. From cell $(i, j)$ it can move to $(i+1, j)$ or $(i, j+1)$ if they exist. The **cost of a path** is the sum of values in all cells along the path.

What is the **minimum possible cost** of a path from $(1, 1)$ to $(n, m)$?
